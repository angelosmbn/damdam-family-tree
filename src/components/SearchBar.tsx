...
function collectNames(node: FamilyNode, sectionId: string, sectionTitle: string, hits: SearchHit[]) {
  for (const p of node.partners) {
    hits.push({ name: p.name, sectionId, sectionTitle });
  }
  node.children?.forEach((c) => collectNames(c, sectionId, sectionTitle, hits));
}
...
export function SearchBar({ onSelect }: SearchBarProps) {
  ...
  const index = useMemo(() => {
    ...
    for (const s of sections) {
      collectNames(s.root, s.id, s.title, hits);
    }
    return hits;
  }, []);
          ...
          {results.map((r, i) => (
            <li key={`${r.sectionId}-${r.name}-${i}`}>
              ...
              >
                ...
                <span className="ml-2 text-xs text-stone-500">{r.sectionTitle}</span>
              </button>
            </li>
          ))}
        </ul>
